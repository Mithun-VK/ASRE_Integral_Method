"""
ASRE Configuration Module (Enhanced v2.0)

Centralized configuration using Pydantic v2 for type safety and validation.

✅ NEW (v2.0): Enhancement parameters for momentum score
- Trend continuation detection
- Adaptive thresholds
- Trade filters (confirmation, holding period)
"""
...
# ---------------------------------------------------------------------------
# Momentum Configuration (ENHANCED v2.0)
# ---------------------------------------------------------------------------

class MomentumConfig(ASREBaseConfig):
    """
    Momentum score configuration with enhancement parameters.

    Original Parameters:
    -------------------
    kappa : float
        Exponential decay rate (default: 0.03)
        Lower = longer memory, Higher = shorter memory

    beta_m : float
        Mean reversion weight for autocorrelation (default: 0.2)
        Lower = less mean reversion, Higher = more mean reversion

    window_60d : int
        Rolling window size in days (default: 60)

    Enhancement Parameters (v2.0):
    ------------------------------
    use_enhancements : bool
        Enable all enhancement features (default: False)
        Set to True to activate trend filters, holding period, etc.

    min_holding_days : int
        Minimum days to hold a position once entered (default: 12)
        Reduces friction by preventing rapid exits

    confirmation_days : int
        Days signal must persist before acting (default: 3)
        Reduces whipsaw trades

    trend_continuation_threshold : float
        Trend strength threshold for exit override (default: 70)
        When trend > threshold, may stay in position despite exit signal

    use_adaptive_thresholds : bool
        Enable dynamic entry/exit thresholds (default: True when enhancements on)

    base_long_threshold : float
        Base threshold for long entry (default: 70)
        Can be dynamically adjusted if use_adaptive_thresholds=True

    base_short_threshold : float
        Base threshold for short entry (default: 30)
        Can be dynamically adjusted if use_adaptive_thresholds=True
    """
    ...
    min_holding_days: int = Field(default=12, ge=0, le=30)
    confirmation_days: int = Field(default=3, ge=0, le=10)
    trend_continuation_threshold: float = Field(default=70.0, ge=50.0, le=100.0)
    use_adaptive_thresholds: bool = Field(default=True)
    base_long_threshold: float = Field(default=70.0, ge=50.0, le=90.0)
    base_short_threshold: float = Field(default=30.0, ge=10.0, le=50.0)
    ...
    def validate_thresholds(self) -> 'MomentumConfig':
        """Validate that long threshold > short threshold."""
        if self.use_enhancements:
            if self.base_long_threshold <= self.base_short_threshold:
                raise ValueError(
                    f"base_long_threshold ({self.base_long_threshold}) must be > "
                    f"base_short_threshold ({self.base_short_threshold})"
                )
        return self

    @classmethod
    def original(cls) -> 'MomentumConfig':
        """
        Original configuration without enhancements.

        Backward compatible with existing code.
        No filters applied.

        Returns:
            MomentumConfig with use_enhancements=False
        """
        return cls(use_enhancements=False)

    @classmethod
    def balanced(cls) -> 'MomentumConfig':
        """
        Balanced configuration with enhancements.

        Good for most stocks and market conditions.
        Expected: 40-50% trade reduction, captures 60%+ of strong trends.

        Best for: MSFT, mixed markets, general use

        Returns:
            MomentumConfig with balanced enhancement settings
        """
        return cls(
            kappa=0.03,
            beta_m=0.2,
            ...
            min_holding_days=12,
            confirmation_days=3,
            trend_continuation_threshold=70.0,
            use_adaptive_thresholds=True,
            base_long_threshold=68.0,
            base_short_threshold=32.0
        )

    @classmethod
    def aggressive(cls) -> 'MomentumConfig':
        """
        Aggressive configuration for maximum trend capture.

        Best for strong trending stocks (like AAPL +72%, NVDA).
        Maximizes trend capture, accepts higher volatility.
        Expected: Captures 70%+ of strong trends, fewer exits.

        Best for: AAPL, NVDA, AMZN (strong trending stocks)

        Returns:
            MomentumConfig with aggressive enhancement settings
        """
        return cls(
            kappa=0.02,              # Longer memory
            beta_m=0.1,              # Less mean reversion
            use_enhancements=True,
            min_holding_days=15,     # Hold longer
            confirmation_days=2,     # Enter faster
            trend_continuation_threshold=65.0,  # Stay in trends
            use_adaptive_thresholds=True,
            base_long_threshold=65.0,  # Easier entry
            base_short_threshold=35.0
        )

    @classmethod
    def conservative(cls) -> 'MomentumConfig':
        """
        Conservative configuration for risk control.

        Best for volatile or uncertain markets.
        Prioritizes drawdown control over trend capture.
        Expected: Lower drawdowns, may miss some trends.

        Best for: TSLA, volatile markets, risk averse

        Returns:
            MomentumConfig with conservative enhancement settings
        """
        return cls(
            kappa=0.05,              # Shorter memory
            beta_m=0.3,              # More mean reversion
            use_enhancements=True,
            min_holding_days=10,
            confirmation_days=5,     # More confirmation
            trend_continuation_threshold=75.0,  # Stricter
            use_adaptive_thresholds=True,
            base_long_threshold=75.0,  # Harder entry
            base_short_threshold=25.0  # Easier exit
        )
...
class CompositeConfig(ASREBaseConfig):
    ...
    w_t_base: float = Field(default=0.3, ge=0.2, le=0.5)
    w_m_base: float = Field(default=0.3, ge=0.1, le=0.4)
...
# ---------------------------------------------------------------------------
# Backtest Configuration (ENHANCED v2.0)
# ---------------------------------------------------------------------------

class BacktestConfig(ASREBaseConfig):
    """
    Backtesting configuration with enhancement support.

    Note: threshold_long and threshold_short are used when
    use_enhancements=False. When enhancements are enabled,
    adaptive thresholds from MomentumConfig are used instead.
    """

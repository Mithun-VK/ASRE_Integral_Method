...
class Config:
    # ========== PATHS ==========
    STATE_FILE = "data/moat/portfolio_state.json"
    TRADE_LOG = "data/moat/trade_history.csv"
    METRICS_LOG = "data/moat/daily_metrics.csv"
    ...
    STOCK_SECTORS = {
        # Technology
        'TSM': 'Technology', 'ASML': 'Technology', 'MSFT': 'Technology',
        'GOOG': 'Technology', 'INTU': 'Technology', 'CRM': 'Technology',
        'ADBE': 'Technology', 'AAPL': 'Technology', 'AVGO': 'Technology',
        'NVDA': 'Technology', 'FTNT': 'Technology', 'META': 'Technology',
        
        # Financials
        'MA': 'Financials', 'V': 'Financials', 'JPM': 'Financials',
        'MCO': 'Financials',
        
        # Consumer
        'AMZN': 'Consumer', 'PEP': 'Consumer', 'PG': 'Consumer',
        'KO': 'Consumer', 'DIS': 'Consumer',
        
        # Industrial
        'DE': 'Industrial', 'SHW': 'Industrial', 'CPRT': 'Industrial',
    ...
    # ========== UNIVERSE ==========
    UNIVERSE = [
        "TSM", "MA", "ASML", "MSFT", "GOOG", "DE", "TMO", "SHW", "CPRT",
        "AMZN", "PEP", "META", "INTU", "PG", "V", "CRM", "JPM", "ADBE",
        "AAPL", "KO", "AVGO", "NVDA", "FTNT", "DIS", "MCO"
    ]
...
class DipAnalyzer:
    @staticmethod
    def analyze_dip(price: float, sma_200: float, asre_score: float, quality_tier: str) -> DipAnalysis:
        ...
        # Quality checks
        if Config.DIP_REQUIRE_A_OR_B_TIER and quality_tier not in ['A', 'B']:
            analysis.approved_for_dip_buy = False
            analysis.reason = f"Tier {quality_tier} not approved (need A/B)"
            return analysis

        if asre_score < Config.DIP_MIN_ASRE:
            analysis.approved_for_dip_buy = False
            analysis.reason = f"ASRE {asre_score:.1f} below minimum {Config.DIP_MIN_ASRE}"
            return analysis

        if distance < Config.DIP_MAX_DISTANCE:
            analysis.approved_for_dip_buy = False
            analysis.reason = f"Too far below SMA ({distance:.1f}% < {Config.DIP_MAX_DISTANCE}%)"
            return analysis

        if analysis.confidence < Config.DIP_MIN_CONFIDENCE:
            analysis.approved_for_dip_buy = False
